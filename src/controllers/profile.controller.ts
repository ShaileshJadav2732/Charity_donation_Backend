...
export const completeDonorProfile = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const { firstName, lastName, phoneNumber, address, city, state, country, bio } = req.body;
    ...
    if (user.role !== 'donor') {
      return res.status(403).json({ message: 'Only donors can complete this profile' });
    }
...
export const completeOrganizationProfile = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ message: 'Unauthorized' });
    }

    const { name, description, phoneNumber, email, website, address, city, state, country } = req.body;

    // Validate required fields
    if (!name || !description || !phoneNumber || !email) {
      return res.status(400).json({ message: 'Name, description, phone number, and email are required' });
    }
    ...
    if (user.role !== 'organization') {
      return res.status(403).json({ message: 'Only organizations can complete this profile' });
    }
